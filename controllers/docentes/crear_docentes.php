<?php
    ...
    if($dni == "" || $nombre == "" || $apellido == "" || $direccion == "" || $usuario == "" || $email == "" || $contrasena == "")
    {
        ...
        header("location:".APP_URL."/vistas/docentes/create.php");
    }
    else
    {
        if ($contrasena == $contrasena2)
        {
            ...
            else
            {
                ...
                //*************** INSERTA EN LA TABLA DOCENTES ***************//
                $sentencia = $pdo->prepare("INSERT INTO docentes(persona_id,especialidad,antiguedad,estado,fyh_creacion) 
                VALUES(:persona_id,:especialidad,:antiguedad,:estado,:fecha)");
                ...
                if($sentencia->execute())
                {
                    $pdo->commit();
                    session_start();
                    $_SESSION['mensaje'] = "El personal docente se registro correctamente.";
                ...
                else
                {
                    $pdo->rollBack();
                    session_start();
                    $_SESSION['mensaje'] = "No se pudo registrar al personal docente, comuníquese con el Administrador.";
                    $_SESSION['icono'] = "error";
